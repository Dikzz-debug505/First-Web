...
const red=mat(0x9d3b2e,.72,0x3d0e08,.08);
...
const floor=new THREE.Mesh(new THREE.BoxGeometry(12,.3,6.8),mat(0x7a5a3c,.85));floor.position.y=1.55;g.add(floor);
...
const moon=new THREE.Mesh(new THREE.SphereGeometry(4.2,48,48),new THREE.MeshBasicMaterial({color:0xd86a58}));
...
const body=new THREE.Mesh(new THREE.CylinderGeometry(.25,.25,.5,8),mat(0xd4a35a,.62,0xc96e25,1.35));body.position.y=1.55;g.add(body);
...
world.add(new THREE.Points(leafGeo,new THREE.PointsMaterial({color:0xc58a56,size:.085,transparent:true,opacity:.62})));
...
camera.position.x=Math.sin(current*Math.PI*1.35)*.65;camera.position.y=6.1+Math.sin(current*Math.PI*1.35)*.75;camera.position.z=24-current*72;
camera.rotation.y=Math.sin(current*Math.PI*2)*.035;camera.rotation.x=-.03+Math.sin(current*Math.PI)*.018;
world.rotation.y=Math.sin(current*Math.PI*1.5)*.012;warm.position.z=-current*45;moon.position.x=12+Math.sin(current*3)*3;
